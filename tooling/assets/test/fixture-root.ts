 ...
 * tree (e.g. `packages/theme/src/scenes/<id>.ts` and `games/<id>/assets/*.png`) without touching
 * the real repo. Call `removeFixtureRoot` when done.
 */
export function createFixtureRoot(files: FixtureFiles): string {
  const rootDir = realpathSync(mkdtempSync(join(tmpdir(), "couchcade-assets-")));

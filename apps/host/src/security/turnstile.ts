// The invisible Turnstile widget behind the Open room button (docs/architecture/security.md, "Where
// Turnstile runs"). Turnstile is the only third-party script on the page, and it loads only when
// the host submits the passcode. apps/controller/src/security/turnstile.ts is the phone's copy:
// apps can't import each other, so keep the two in step.

/** The bot check `POST /api/rooms` carries. */
export interface TurnstileProvider {
  /** A fresh single-use token, requested when the host submits the passcode. */
...
/** A run that hasn't produced a token by now fails, so the Open room button never waits forever. */
...
export function createTurnstile({
  ...
  return {
    async token() {
      ...
      api = turnstile;
      // A run still waiting (a second submit) gives way to this one.
...
/** Adds Cloudflare's script to the page once. A failed load is tried again on the next submit. */

/**
 * Whether this phone can trust the room clock yet (the same guard Quick Draw, Target Range and
 * Strike Night use). Before that, a swing's `at` would be stamped against an untrusted offset, so
 * `present.ts` keeps the big action off until it's true.
 *

...
import { bandejaCueEvent } from "../../src/host/cues.ts";
import type { BandejaCue } from "../../src/host/cues.ts";
import type BandejaScene from "../../src/host/scene.ts";
import type { BandejaInput, BandejaState } from "../../src/shared/index.ts";
 ...
 * runtime/stage.ts): a canvas with nearest-neighbour pixels and the house style fonts loaded, the
 * scene added under the game id with `HostSceneData`. The stage draws the 480×270 world at a
 * whole-number zoom and the overlays at the canvas resolution. Tests drive Phaser by hand, one
 * game tick per frame -- through the real, `withRewind`-wrapped `game` export, so `getState()`
 * returns exactly what the host runtime hands the shipped scene (`Rewound<BandejaState>`).
 */
...
export async function startScene(
  ...
  const data: HostSceneData<Rewound<BandejaState, BandejaInput>> = {
  ...
  const scene = stage.scene.getScene(game.id) as BandejaScene;
  const cues: BandejaCue[] = [];
  scene.events.on(bandejaCueEvent, (cue: BandejaCue) => cues.push(cue));
  return {
    stage,
    scene,
    cues,

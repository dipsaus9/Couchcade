...
/**
 * A full swing shape for `emitOn: "peak"`: a burst at `gamma` deg/s for `durationMs`, then a quiet
 * tail (well under Putt Club's own `SWING_END_RATE`, 40 deg/s) long enough for the detector's own
 * end-quiet window to close the swing naturally, the way a real putting stroke decelerates after
 * the strike.
 */
export function puttBurst(from: number, durationMs: number, gamma: number): MotionSample[] {
  return [...turn(from, durationMs, gamma), ...turn(from + durationMs + 16, 150, 0)];
}

...
import { joinRoom } from "./flows.ts";
import { playMotionTrace, type MotionTrace } from "./motion.ts";

// Target Range bots for E2E specs (docs/games/target-range.md), shared by games/target-range.spec.ts
// and platform/realtime-link.spec.ts (CC-3.22). The phone reads the target and the wind from the TV,
// so the bots watch the TV's game state the same way Quick Draw's bots do (src/quick-draw.ts):
// `watchTv` wraps the host's stage in the Vite dev build to read the running game's state.
...
export async function volleyOpen(host: Page, round: number, arrow: number): Promise<TvState> {
...
export async function pullFullDraw(phone: Page, arrow: number): Promise<void> {
 ...
 */
export async function aimAndShoot(phone: Page, state: TvState): Promise<void> {
...
export function holdStill(phone: Page): { stop(): Promise<void> } {
  let holding = true;
  const done = (async () => {
 ...
 */
export async function startMatch(host: Page, ana: Page, ben: Page, code: string): Promise<void> {
 ...
 */
export async function playFullMatch(
  host: Page,
  ana: Page,
  ben: Page,

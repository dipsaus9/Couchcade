...
| 7 | No STUN and no TURN server (owner) | The link only uses local candidates the phone and the laptop can already reach — usually names found on the same Wi-Fi, but not only that (see [Who connects directly](#who-connects-directly)). Nothing about the link leaves the house. Phones on 4G, on guest Wi-Fi or on a network that keeps devices apart usually fall back to today's path, though CC-3.13's spike found 4G can also connect directly. No paid relay, no card. |
...
1. **No STUN server (not the recommendation).** Connections use `iceServers: []`: only local host candidates, which browsers name with random mDNS names. Nothing about the link leaves the house. The draft recommended Cloudflare's free STUN server. Phones connect directly when the network lets their host candidates find each other — usually the same Wi-Fi as the laptop, by local name, but not only then: CC-3.13's spike also connected a phone on 4G to the laptop on Wi-Fi, most likely because both had a globally-routable IPv6 address. That isn't guaranteed on every network. Everything that can't find the other side uses the relay path. See [Who connects directly](#who-connects-directly).
...
5. **Internet play.** The link isn't built to reach a phone anywhere on the internet: no STUN, no TURN, no signalling for that. A phone on a different network from the TV — 4G while the TV is on Wi-Fi, say — can still end up connecting directly when both sides' host candidates can already reach each other, such as a globally-routable IPv6 address on both (CC-3.13's spike evidence, see [Who connects directly](#who-connects-directly)). That's opportunistic, not designed for, and not guaranteed. A phone that can't connect this way uses the relay path, as today.
...
With no STUN server (owner answer 1), a phone and the laptop only offer host candidates: addresses gathered locally, with no server confirming what they look like from outside. The connection works whenever at least one side can reach the other's host candidate. On the same local network that's usually by resolving a random mDNS name; the other side then learns the real local address from the incoming connection checks, a peer-reflexive candidate ([IETF draft](https://www.ietf.org/archive/id/draft-ietf-mmusic-mdns-ice-candidates-03.html)). It isn't only the same network, though. CC-3.13's spike ran the link on the owner's iPhone and laptop, real devices, no STUN: on the TV's home Wi-Fi it connected in 2410 ms with a host/host candidate pair; on 4G — a different network from the laptop's Wi-Fi — it also connected, in 580 ms, also a host/host pair. The likely reason is that both devices had a globally-routable IPv6 address, which let their host candidates reach each other straight across networks and NATs. That isn't guaranteed: a network without IPv6, or a NAT or firewall that keeps host candidates from finding each other, still falls back to the relay path. So the same Wi-Fi is the reliable common case, not a hard requirement. mDNS uses multicast, which some networks block ([BlogGeek.me](https://bloggeek.me/psa-mdns-and-local-ice-candidates-are-coming/)).
...
| 4G or 5G, TV on Wi-Fi | Can be direct: CC-3.13's spike connected a phone on 4G to the laptop on Wi-Fi (580 ms, host/host pair), most likely because both had a globally-routable IPv6 address. Otherwise relay. |
...
The mDNS draft's own field test saw a small drop in connections when both sides used mDNS names, and more connections leaning on STUN ([IETF draft](https://www.ietf.org/archive/id/draft-ietf-mmusic-mdns-ice-candidates-03.html)). Those numbers come from WebRTC traffic across the internet, not from a phone next to a laptop at home, so they don't predict our rate. CC-3.13's spike measured the no-STUN link on the owner's own iPhone and laptop: it connected every time it was tried, both on the home Wi-Fi and on 4G, both times with a direct host/host candidate pair (see above). The owner deferred iCloud Private Relay, Low Power Mode and Android runs for this round ("I don't care about icloud, lower power mode or android for now," 2026-09-18); those rows in [Browser support](#browser-support) stay unverified. Every phone that doesn't connect plays exactly as it does today, with the better smoothing.

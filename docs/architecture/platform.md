...
6. `apps/host` never imports a game's `src/controller/`, and the controller app never imports a game's `src/host/`. The host reaches game code only through the game's `src/index.ts` and its `hostScene` loader. The phone reaches game code only through the game's controller entry `src/controller/index.ts` (owner decision, 16 September 2026).
7. Inside a game:
   ...
   - `src/controller/` may import `shared/`, `ui`, `motion`, `theme`, `game-sdk` and `vue`. Never `phaser`, `stage`, `audio` or `physics`. Nothing under `src/controller/` may reach `planck`, `@couchcade/physics` or `phaser`, not even through `shared/`; dependency-cruiser enforces this, so Planck.js never ships to phones.
   - `src/index.ts` imports only `shared/` and `game-sdk`. It reaches `host/` only through dynamic `import()`, and never imports `controller/`.
...
|---|---|---|
| `meta` | 1: code, created_at, locked, phase, host_seen_at, host_revoked | Create, lock, phase change, host leaves, host flood revocation |
...
│   └── index.ts        # phone entry: export default defineController({ id, component: () => import("./Controller.vue") })
...
// games/<id>/src/controller/index.ts: the phone entry (owner decision, 16 September 2026)
export interface CouchcadeController {
  id: string;                                                // same id as the game
  component: () => Promise<Component>;                       // () => import("./Controller.vue").then((m) => m.default)
}
...
4. **Sharp text at any zoom (CC-10.10).** The overlay zoom is only ×1 on a fullscreen 1080p canvas. A 1080p TV in a browser window (about 1920×970) gets a ×3 world and a ×0.75 overlay, a 720p screen ×0.5, a HiDPI laptop ×1.25 to ×1.75. So before every frame `StageScene` (`sharpenText`) makes each overlay text land 1:1 on canvas pixels:
   - its glyphs rasterise at exactly the overlay zoom, below ×1 too. 1080p glyphs shrunk by nearest-neighbour sampling lose and double strokes, which looked blurry and odd on the TV (owner, 17 September 2026);
   - text that isn't rotated or scaled snaps to whole canvas pixels (`vertexRoundMode` `full`), because Phaser only rounds under an unzoomed camera;
   - its texture is a whole number of canvas pixels: the right and bottom padding grow by less than one canvas pixel, so snapping never stretches a text by a pixel.

   Rotated or scaling text, such as a callout, keeps Phaser's default rounding.
5. **Fonts.** Phaser measures and draws text once, when the text is created. `phaserStage.start` (`apps/host/src/runtime/stage.ts`) waits for Fredoka and Pixelify Sans in every weight the TV type scale uses before it starts a scene. It waits 3 s at most, so a missing font never blocks a game.
...
// apps/controller/src/runtime/games.ts: lazy, the phone loads one controller entry when a game starts
import { createControllerRegistry } from "@couchcade/game-sdk/registry";
export const registry = createControllerRegistry(
